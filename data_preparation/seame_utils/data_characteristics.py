...
def _has_hesitations_and_discourse_particles(text):
    # Extracted from unfiltered transcript, annotated as (<hes>) and [<dis>]
    hesitation_pattern = r'\(.*?\)'
    discourse_token_pattern = r'\[.*?\]'
    return bool(re.search(hesitation_pattern, text.lower())) or bool(re.search(discourse_token_pattern, text.lower()))


def _is_incomplete_utterance(text):
    # Incomplete utterances, trailing off
    # Extracted from unfiltered transcript, annotated as <sentence>~
    pattern = r'~'
    return bool(re.search(pattern, text.lower()))
...
def _has_foreign_language_tokens(text):
    ...
    # Extracted from unfiltered transcript, annotated as #<word>#
    pattern = r'\#.+?\#'
    return bool(re.search(pattern, text.lower()))
...
def _has_unknown(text):
    ...
    # Extracted from unfiltered text, annotated as "<unk>"
    pattern = r'<unk>'
    return bool(re.search(pattern, text.lower()))
