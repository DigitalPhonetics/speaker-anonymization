# based on IMSToucan/Modules/ToucanTTS/dit.py
...
class DiTConVBlock(nn.Module):
    ...
    def forward(self, x, c, x_mask):
        ...
        if c is not None:
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).transpose(1, 2).chunk(6, dim=1)  # shape: [batch_size, channel, 1]
