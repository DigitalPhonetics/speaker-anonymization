# based on IMSToucan/Modules/ToucanTTS/InferenceToucanTTS.py
...
from anonymization.modules.tts.toucan_codeswitching.CSConformer import CSConformer
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.LengthRegulator import LengthRegulator
from anonymization.modules.tts.IMSToucan.Preprocessing.articulatory_features import get_feature_to_index_lookup
from anonymization.modules.tts.IMSToucan.Utility.utils import make_non_pad_mask

from anonymization.modules.tts.toucan_codeswitching.CSflow_matching import CFMDecoder

class CSToucanTTS(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if integrate_language_embedding_into_encoder_out:
            utt_embed_dim = utt_embed_dim + lang_emb_size  # 192 + 32
        ...
        self.encoder = CSConformer(conformer_type="encoder",
                                 ...
                                 zero_triu=False,
                                 utt_embed=utt_embed_dim,  # 224
                                 lang_embs=lang_embs,  # 8000
                                 lang_emb_size=lang_emb_size, #32
        ...
        self.decoder = CSConformer(conformer_type="decoder",
                                 attention_dim=attention_dimension,
                                 attention_heads=attention_heads,
    ...
    def _forward(self,
        ...
        if utterance_embedding is not None:
            ...
            if self.integrate_language_embedding_into_encoder_out and lang_ids is not None:
                ...
                for lang_id in lang_ids:
                    lang_embs = self.encoder.language_embedding(lang_id.unsqueeze(0))  # shape 1, 32
                    lang_embs = torch.nn.functional.normalize(lang_embs)
                    utterance_embeddings.append(torch.cat([lang_embs, utterance_embedding], dim=1).detach()) # shape 1, 224
                utterance_embeddings = torch.concatenate(utterance_embeddings, dim=0).unsqueeze(0)  # keep batch dimension
            else:
                utterance_embeddings = utterance_embedding.unsqueeze(0)
        else:
        ...
        encoded_texts, _ = self.encoder(text_tensors, text_masks, utterance_embeddings=utterance_embeddings, lang_ids=lang_ids)
        ...
        pitch_predictions = self.pitch_predictor(mu=reduced_pitch_space,
                                                 ...
                                                 c=utterance_embeddings) if gold_pitch is None else gold_pitch
        ...
        energy_predictions = self.energy_predictor(mu=reduced_energy_space,
                                                   ...
                                                   n_timesteps=10,
                                                   temperature=prosody_creativity,
                                                   c=utterance_embeddings) if gold_energy is None else gold_energy
        ...
        predicted_durations = torch.clamp(torch.ceil(self.duration_predictor(mu=reduced_duration_space,
                                                                             ...
                                                                             n_timesteps=10,
                                                                             temperature=prosody_creativity,
                                                                             c=utterance_embeddings)), min=0.0).long().squeeze(1) if gold_durations is None else gold_durations
        ...
        upsampled_utterance_embeddings = self.length_regulator(utterance_embeddings, predicted_durations)

        # decoding spectrogram
        decoded_speech, _ = self.decoder(upsampled_enriched_encoded_texts, None, utterance_embeddings=upsampled_utterance_embeddings)
