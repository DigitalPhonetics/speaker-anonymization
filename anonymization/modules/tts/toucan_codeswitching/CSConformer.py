# based on IMSToucan/Modules/GeneralLayers/Conformer.py
"""
...
sys.path.insert(0, str((Path(__file__).parent.parent / 'IMSToucan').absolute()))

from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.Attention import RelPositionMultiHeadedAttention
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.ConditionalLayerNorm import AdaIN1d
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.ConditionalLayerNorm import ConditionalLayerNorm
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.Convolution import ConvolutionModule
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.EncoderLayer import EncoderLayer
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.LayerNorm import LayerNorm
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.MultiLayeredConv1d import MultiLayeredConv1d
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.MultiSequential import repeat
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.PositionalEncoding import RelPositionalEncoding
from anonymization.modules.tts.IMSToucan.Modules.GeneralLayers.Swish import Swish
from anonymization.modules.tts.IMSToucan.Utility.utils import integrate_with_utt_embed

from anonymization.modules.tts.toucan_codeswitching.CSutils import CSAdaIN1d


class CSConformer(torch.nn.Module):
    """
    ...
    def __init__(self, conformer_type, attention_dim=256, attention_heads=4, linear_units=2048, num_blocks=6, dropout_rate=0.1, positional_dropout_rate=0.1,
        ...
        super(CSConformer, self).__init__()
        ...
        if utt_embed is not None:
            if conformer_type == "encoder":  # the encoder gets an additional conditioning signal added to its output
                if embedding_integration == "AdaIN":
                    self.encoder_embedding_projection = CSAdaIN1d(style_dim=utt_embed, num_features=attention_dim)
                ...
                if embedding_integration == "AdaIN":
                    self.decoder_embedding_projections = repeat(num_blocks, lambda lnum: CSAdaIN1d(style_dim=utt_embed, num_features=attention_dim))
                elif embedding_integration == "ConditionalLayerNorm":
    ...
    def forward(self,
        ...
        if lang_ids is not None:
            lang_embs = self.language_embedding(lang_ids.unsqueeze(0))
            projected_lang_embs = self.language_embedding_projection(lang_embs)
                ...
                if isinstance(xs, tuple):
                    ...
                    if self.conformer_type != "encoder":
                        x = integrate_with_utt_embed(hs=x,
                                                     utt_embeddings=utterance_embeddings,
